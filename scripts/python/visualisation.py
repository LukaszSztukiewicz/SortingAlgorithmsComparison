...
EX1 = ["BS", "HS", "CS", "ShS"]
for alg in EX1:
    ax.plot(df[df.algorithm == alg].n, smoothen(
        df[df.algorithm == alg].time, 15), label=alg)
...
for types in zip(TYPES[1::2], TYPES[::2]):
    ...
    for t in types:
        for alg in EX2:
            ax.plot(df[(df.algorithm == alg) & (df.type == t)].n, smoothen(
                df[(df.algorithm == alg) & (df.type == t)].time, 15), label=f'{alg}_{t}')
    ax.set_xlabel("number of elements")
    ax.set_ylabel("time to sort")
    ax.set_ylim([0, 0.15])
    ax.legend(loc='best')
